...
public sealed class PagedRequestTests
{
    ...
    [Fact]
    public void Offset_stays_non_negative_for_the_int_max_page_boundary()
    {
        var request = new PagedRequest(page: int.MaxValue, pageSize: 100);

        long expected = (int.MaxValue - 1L) * 100;

        Assert.Equal(expected, request.Offset);
        Assert.True(request.Offset >= 0);
    }
