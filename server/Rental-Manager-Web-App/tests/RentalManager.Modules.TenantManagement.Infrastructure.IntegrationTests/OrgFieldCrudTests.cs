...
public sealed class OrgFieldCrudTests
{
    ...
    [Fact]
    public async Task List_filters_by_active_flag()
    {
        await _fixture.ResetOrgDataAsync();
        await using TenantScope tenant = OrganizationA();

        FieldDto active = await tenant.CreateTextFieldAsync("room_number");
        FieldDto inactive = await tenant.CreateTextFieldAsync("floor");

        await tenant.UpdateAsync(inactive.Id, new UpdateFieldRequest
        {
            Name = inactive.Name,
            IsActive = false,
        ...
        PagedResult<FieldDto> activeOnly = await tenant.ListAsync(new GetFieldsRequest
        {
            IsActive = true
        });

        Assert.Equal(active.Key, Assert.Single(activeOnly.Items).Key);
    }
    ...
    public async Task Paging_is_stable_when_the_sort_columns_are_identical()
    {
        ...
        // Same Name across every row, so only the [Id] tie-breaker can make the
        // order deterministic.
