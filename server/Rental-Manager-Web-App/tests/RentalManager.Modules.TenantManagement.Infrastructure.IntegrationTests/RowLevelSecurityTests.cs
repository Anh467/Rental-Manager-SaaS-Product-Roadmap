...
public sealed class RowLevelSecurityTests
{
    ...
    public async Task Another_organization_cannot_read_the_field()
    {
        ...
        // 404 rather than 403, so the other tenant learns nothing about whether
        // the resource exists.
    ...
    public async Task Same_key_in_two_organizations_is_allowed()
    {
        ...
        Assert.True(fromA.IsPrimaryDisplayField);
        Assert.True(fromB.IsPrimaryDisplayField);
    }
    ...
    public async Task Database_blocks_a_write_that_belongs_to_another_organization()
    {
        ...
        SqlException exception = await Assert.ThrowsAsync<SqlException>(
            () => connection.ExecuteAsync(
                ...
                INSERT INTO [org].[Field]
                    ([Id], [OrganizationId], [TargetEntityType], [Key],
                     [NormalizedKey], [Name], [FieldTypeId], [CreatedAt], [UpdatedAt])
                VALUES
                    (@Id, @OrganizationId, N'Room', N'smuggled', N'SMUGGLED',
                     N'Smuggled', 1, @Now, @Now);
                """,
                new
                {
        ...
        // 33504: the target object has a block predicate that disallows the write.
